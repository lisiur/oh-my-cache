(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    var Store = /** @class */ (function () {
        ...
        Store.prototype.registerConfig = function (name, params) {
            if (params === void 0) { params = {}; }
            var _a = params.expire, expire = _a === void 0 ? 0 : _a, _b = params.dependencies, dependencies = _b === void 0 ? [] : _b;
            var config = this.store.get(name);
            if (!config) {
                throw new Error(name + " hasn't be registed");
            }
            this.store.set(name, __assign(__assign({}, config), { expire: expire, triggers: new Set(), cache: new Map() }));
            this.setDependencies(name, new Set(dependencies));
        };
        Store.prototype.registerObj = function (obj, configs) {
            var _this = this;
            if (configs === void 0) { configs = []; }
            var res = {};
            Object.entries(obj).forEach(function (_a) {
                var name = _a[0], fn = _a[1];
                _this.register(name, fn.bind(obj));
                res[name] = _this.callFn(name);
            });
            configs.forEach(function (config, i) {
                var name = config.name, cfg = __rest(config, ["name"]);
                if (!name) {
                    throw new Error("config[" + i + "] doesn't has name");
                }
                _this.registerConfig(name, cfg);
            });
            return res;
        };
        ...
        Store.prototype.setDependencies = function (name, dependencies) {
            ...
            dependencies.forEach(function (dependency) {
                if (typeof dependency === 'string') {
                    ...
                    var store = dependency.store.store;
                    if (!store.get(dependency.name)) {
                        throw new Error(dependency + " hasn't be registed");
                    }
                    store.get(dependency.name).triggers.add({ name: name, store: _this });
                }
            });
        };
        /**
